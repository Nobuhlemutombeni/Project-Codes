...
gmodAo <- gam(OAH_num ~ s(Air_Temp), data=data_train,family=ocat(R=5))
summary(gmodAo)
plot(gmodAo)
gam.check(gmodAo)
...
datapred <- predict.gam(gmodAo, data=data_test,type = "response")
View(datapred)

# this shows how probability of falling into each risk category changes as a function 
# of air temp
data1 <- data_test %>% filter(!is.na(OAH_num), !is.na(Air_Temp))
mycols <- levels(data1$OAH)
ocat_model <- gmodAo
ocat_predict = predict(ocat_model,newdata = data_test,type = "response")
colnames(ocat_predict) = mycols
ocat_predict = as.data.frame(ocat_predict)%>%
  mutate(x= data1$Air_Temp,y=factor(data1$OAH_num, levels = 1:5, labels = mycols)) %>%
...
#===================ordered categorical response is probably better for this data====================3
par(mfrow=c(1,1))
gmodWS <- gam(OAH_num ~ s(Wind_Speed), data=data_train,family=ocat(R=5))
summary(gmodWS)
plot(gmodWS)
gam.check(gmodWS)
...
# predictions for ocat responses are probabilities of falling into each category
datapred1 <- predict.gam(gmodWS,data=data_test, type = "response")
View(datapred1)

# this shows how probability of falling into each risk category changes as a function 
# of air temp
data2 <- data_test %>% filter(!is.na(OAH_num), !is.na(Wind_Speed))
mycols <- levels(data2$OAH)
ocat_model <- gmodWS
ocat_predict = predict(ocat_model,newdata=data_test,type = "response")
colnames(ocat_predict) = mycols
ocat_predict = as.data.frame(ocat_predict)%>%
  mutate(x= data2$Wind_Speed,y=factor(data2$OAH_num, levels = 1:5, labels = mycols)) %>%
  #mutate(x= fitted(ocat_model),y=as.numeric(dat$OAH_num))%>%
  gather(pred_level,prediction,mycols) %>%
  mutate(obs_val = as.numeric(y==pred_level)) %>%
  mutate(pred_level = factor(pred_level, levels = mycols))
...
#============================ ordered categorical response is probably better for this data
gmodC <- gam(OAH_num ~ s(Cloud), data=data_train,family=ocat(R=5))
summary(gmodC)
plot(gmodC)
gam.check(gmodC)
...
# predictions for ocat responses are probabilities of falling into each category
datapred3 <- predict.gam(gmodC, data=data_test,type = "response")
View(datapred3)

# this shows how probability of falling into each risk category changes as a function 
# of air temp
data4 <- data_test %>% filter(!is.na(OAH_num), !is.na(Cloud))
mycols <- levels(data4$OAH)
ocat_model <- gmodC
ocat_predict = predict(ocat_model,newdata=data_test,type = "response")
colnames(ocat_predict) = mycols
ocat_predict = as.data.frame(ocat_predict)%>%
  mutate(x= data4$Cloud,y=factor(data4$OAH_num, levels = 1:5, labels = mycols)) %>%
  #mutate(x= fitted(ocat_model),y=as.numeric(dat$OAH_num))%>%
  gather(pred_level,prediction,mycols) %>%
  mutate(obs_val = as.numeric(y==pred_level)) %>%
  mutate(pred_level = factor(pred_level, levels = mycols))
...
ggplot(aes(x= x, y= obs_val),data=ocat_predict)+
  ...
  geom_point()+
  geom_line(aes(y= prediction))+
  xlab("Cloud") + ylab("Probability") +
  geom_density(aes(colour = y))

#============================ ordered categorical response is probably better for this data
gmodP <- gam(OAH_num ~  (Precip_Code1), data=data_train,family=ocat(R=5))  #not working with s , work without s but do not plot , works with te and plots also
summary(gmodP)
plot(gmodP)
gam.check(gmodP)
...
#============================ ordered categorical response is probably better for this data
gmodR <- gam(OAH_num ~  (Rain_at_900), data=data,family=ocat(R=5))
summary(gmodR)
plot(gmodR)
gam.check(gmodR)
...
#============================ ordered categorical response is probably better for this data
#gmodX <- gam(OAH_num ~  s(X,Y), data=data,family=ocat(R=5))
gmodX <- gam(OAH_num ~  s(X,Y), data=data_train,family=ocat(R=5))
summary(gmodX)
plot(gmodX)
gam.check(gmodX)
...
# predictions for ocat responses are probabilities of falling into each category
datapred6 <- predict.gam(gmodX,data=data_test, type = "response")
#View(datapred4)

# this shows how probability of falling into each risk category changes as a function 
# of air temp
data7 <- data_test %>% filter(!is.na(OAH_num), !is.na(X),!is.na(Y))
mycols <- levels(data7$OAH)
ocat_model <- gmodX
ocat_predict = predict(ocat_model,newdata=data_test,type = "response")
colnames(ocat_predict) = mycols
ocat_predict = as.data.frame(ocat_predict)%>%
  mutate(x= data7$X,y=factor(data7$OAH_num, levels = 1:5, labels = mycols)) %>%
  #mutate(x= fitted(ocat_model),y=as.numeric(dat$OAH_num))%>%
  gather(pred_level,prediction,mycols) %>%
  mutate(obs_val = as.numeric(y==pred_level)) %>%
  mutate(pred_level = factor(pred_level, levels = mycols))
...
ggplot(aes(x= x, y= obs_val),data=ocat_predict)+
  ...
  geom_point()+
  geom_line(aes(y= prediction))+
  xlab("XY") + ylab("Probability") +
  theme_grey()+geom_density(aes(colour = y))



#============================ ordered categorical response is probably better for this data

gmodT <- gam(OAH_num ~  s(days_since_19Nov), data=data_train,family=ocat(R=5))
summary(gmodT)
plot(gmodT)
gam.check(gmodT)
...
#Just checking for one variable
#==================================================================================================================
gmodX <- gam(OAH_num ~  s(X,Y), data=data_train,family=ocat(R=5))
gmodX1 <- gam(OAH_num ~  te(X,Y)+s(X)+s(Y), data=data_train,family=ocat(R=5))
gmodR <- gam(OAH_num ~  (Rain_at_900), data=data_train,family=ocat(R=5))
gmodP <- gam(OAH_num ~  (Precip_Code), data=data_train,family=ocat(R=5)) 
gmodAo<- gam(OAH_num ~  s(Air_Temp), data=data_train,family=ocat(R=5))
gmodWS<- gam(OAH_num ~  s(Wind_Speed), data=data_train,family=ocat(R=5))
gmodC<- gam(OAH_num ~  s(Cloud), data=data_train,family=ocat(R=5))
gmodWD<- gam(OAH_num ~  s(Wind_Dir), data=data_train,family=ocat(R=5))
gmodD <- gam(OAH_num) ~ s(day_since_19Nov, data=data_train,family=ocat(R=5))
AIC(gmodX,gmodX1,gmodR,gmodP,gmodAo,gmodWS,gmodC,gmodWD)
...
# concurvity check False since the parameters are high
#These show the degree to which each variable is predetermined by each other variable, rather than all the other variables. This can be used to pinpoint which variables have a close relationship. Once again,
#the function returns three measures, this time as three matrices. 
...
#model1 <- gam(OAH_num ~  s(Air_Temp)+s(Wind_Speed,bs="cc",k=16)+s(Wind_Dir,Cloud,bs="fs")+(Rain_at_900)+s(X,Y)+(Precip_Code1)+s(days_since_19Nov),  data=data_train,family=ocat(R=5))
par(mfrow=c(2,2))
summary(model8)
gam.check(model8)
par(mfrow=c(1,1))

plot(model8,shade=TRUE,select=1,se= TRUE,rug = T,shift=coef(model8)[9],all.terms = T,shade.col="blue",main = "Partial effect plot for Wind speed",xlab = "Wind Speed", ylab ="Smooth for Wind Speed s(Wind speed,edf=8.75)" )
plot(model8,shade=TRUE,select=2,scheme=2,all.terms = T,main = "Interaction Between Temperature and Days",xlab = "Air Temperature", ylab =" Avalanche Days" )
plot(model8,select=3,all.terms = T,main = "Preciptation",xlab = "Precipitation", ylab ="Partial for Preciptation" )
#plot(model8,shade=TRUE,all.terms = T,select=3,)
#plot(model8,shade=TRUE,pages=1,se=T,scheme=2,cex=1,all.terms = T)

concurvity(model8, full = TRUE)

model8$family$getTheta(TRUE) ## the estimated cut points


#==============================================================================


# test model gammodel8 on test data
